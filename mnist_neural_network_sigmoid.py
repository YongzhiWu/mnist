...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct  8 13:59:10 2018
...
def train(X_train, y_train, train_images, train_labels, test_images, test_labels):
    ...
    for global_step in range(TRAINING_STEPS):
        ...
        layer3_delta = z_delta.dot(weights4.T) * (layer3 * (1 - layer3))
        layer2_delta = layer3_delta.dot(weights3.T) * (layer2 * (1 - layer2))
        layer1_delta = layer2_delta.dot(weights2.T) * (layer1 * (1 - layer1))
...
def evaluate(X, y, weights1, biases1, weights2, biases2, weights3, biases3, weights4, biases4):
    ...
    layer1 = sigmoid((np.dot(X, weights1) + np.dot(X_b, biases1)) )
    layer2 = sigmoid((np.dot(layer1, weights2) + np.dot(X_b, biases2)) )
    layer3 = sigmoid((np.dot(layer2, weights3) + np.dot(X_b, biases3)) )
    z = np.dot(layer3, weights4) + np.dot(X_b, biases4)
    p = softmax(z)
    y_predict = []
    for row in p:
...
def run(train_images, train_labels, test_images, test_labels):
    ...
    '''
    np.save("./mnist_sigmoid_network_weights/w1.npy", w1)
    np.save("./mnist_sigmoid_network_weights/b1.npy", b1)
    np.save("./mnist_sigmoid_network_weights/w2.npy", w2)
    np.save("./mnist_sigmoid_network_weights/b2.npy", b2)
    np.save("./mnist_sigmoid_network_weights/w3.npy", w3)
    np.save("./mnist_sigmoid_network_weights/b3.npy", b3)
    np.save("./mnist_sigmoid_network_weights/w4.npy", w4)
    np.save("./mnist_sigmoid_network_weights/b4.npy", b4)
    '''
